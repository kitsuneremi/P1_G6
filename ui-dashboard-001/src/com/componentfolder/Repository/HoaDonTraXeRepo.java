...
public class HoaDonTraXeRepo {
    ...
    public ArrayList<HopDongViewModel> getlisthd() {
        try ( Connection conn = DBConnections1.getConnection()) {
            ...
            String sql = "select hopdong.mahopdong,idnv,hopdong.makh,ngaytao,ngayhethan,tinhtrang,ten from hopdong join khachhang on hopdong.makh = khachhang.makh";
            PreparedStatement ps = conn.prepareStatement(sql);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
    ...
    public boolean insert(HoaDonTraXeModel hdhtm) {
        try ( Connection conn = DBConnections1.getConnection()) {
            String sql = "insert into hoadontraxe(mahoadon,mahopdong,ngaytra,mavipham,phiphatsinh,thanhtien) values(?,?,?,?,?,?)";
            PreparedStatement ps = conn.prepareStatement(sql);
    ...
    public boolean update(HoaDonTraXeModel hdhtm) {
        try ( Connection conn = DBConnections1.getConnection()) {
            String sql = "update hoadontraxe set mahopdong = convert(uniqueidentifier,?),ngaytra = ?, vipham = ?, phiphatsinh = ?, thanhtien = ? where mahoadon = ?";
            PreparedStatement ps = conn.prepareStatement(sql);
