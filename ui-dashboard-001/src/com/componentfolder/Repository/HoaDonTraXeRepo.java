...
public class HoaDonTraXeRepo {
    ...
    public ArrayList<HoaDonTraXeViewModel> getlistdata() {
        try {
            ...
            while (rs.next()) {
                ...
                System.out.println(rs.getString(2) + "cc");
    ...
    public boolean update(HoaDonTraXeModel hdhtm) {
        try ( Connection conn = DBConnections1.getConnection()) {
            String sql = "update hoadontraxe set mahopdong = ?,ngaytra = ?, vipham = ?, phiphatsinh = ?, thanhtien = ?, mota = ? where mahoadon = ?";
            PreparedStatement ps = conn.prepareStatement(sql);
    ...
    public double tienthuexe(String idhd) {
        ...
        try ( Connection conn = DBConnections1.getConnection()) {
            double tienthuexe = 0;
            String sql = "select loaiXe.giaThue from hopdong join chitiethopdong on hopdong.mahopdong = chiTietHopDong.mahopdong join chiTietXe on chiTietHopDong.bienSo = chiTietXe.bienSo join loaiXe on loaiXe.malx = chiTietXe.malx where hopDong.mahopdong = ?";
            PreparedStatement ps = conn.prepareStatement(sql);
            ps.setString(1,idhd);
            ResultSet rs = ps.executeQuery();
            while(rs.next()){
